...
?>
<div <?php echo $anchor; ?> class="reference-meta-block-container grid lg:grid-cols-2 gap-8 <?php echo esc_attr($class_name); ?>">
	<?php if (have_rows('office_cards_repeater')): 
		while (have_rows('office_cards_repeater')) : the_row();
			...
			<div class="items-start bg-acryl-beige-lighter flex gap-4 rounded-lg">
				...
				<dl>
					...
					<?php endif; ?>
					<?php if ($excerpt) : ?>
						<dd class="text-lg text-black"><?php 
						$allowed_html = array(
							'a' => array(
								'href' => array(),
								'target' => array(),
						...
						$excerpt = preg_replace('/<a(.*?)>/i', '<a$1 class="underline">', $excerpt);
						echo wp_kses($excerpt, $allowed_html); 
					?></dd>
