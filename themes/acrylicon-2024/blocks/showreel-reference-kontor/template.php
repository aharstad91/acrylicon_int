...
$acf_field_options = [
	...
	'showreel-reference-kontor' => 'referanser-kontor' // Endret til riktig taksonomi
];
...
if ($term_id) {
	...
	if (!is_wp_error($term) && $term) {
		...
		$all_taxonomies = ['referanser-kategorier', 'referanser-produkter', 'referanser-type', 'referanser-kontor'];
				...
				<?php if (has_post_thumbnail()): ?>
					<div class="mb-4">
						<div class="block relative">
							...
							$kontor_terms = get_the_terms($post_id, 'referanser-kontor');
							$produkt_terms = get_the_terms($post_id, 'referanser-kategorier'); // Produktområdene
							
							// Fjern Acrylicon tags fra visningen og ta vare på eventuelle "Produktområde" termer 
							if ($type_terms && !is_wp_error($type_terms)) {
								$filtered_type_terms = array();
								foreach ($type_terms as $term) {
									if (strpos($term->name, 'Acrylicon') === false) {
										$filtered_type_terms[] = $term;
									}
								}
								$type_terms = $filtered_type_terms;
							}
							
							if ($kontor_terms && !is_wp_error($kontor_terms)) {
								$filtered_kontor_terms = array();
								foreach ($kontor_terms as $term) {
									if (strpos($term->name, 'Acrylicon') === false) {
										$filtered_kontor_terms[] = $term;
									}
								}
								$kontor_terms = $filtered_kontor_terms;
							}
							
							if (($post_terms && !is_wp_error($post_terms)) || ($type_terms && !is_wp_error($type_terms)) || 
								($product_terms && !is_wp_error($product_terms)) || ($kontor_terms && !is_wp_error($kontor_terms))): ?>
								<div class="mb-2 absolute">
									...
									// Altid vis produktområder hvis de finnes
									if ($product_terms && !is_wp_error($product_terms)) {
										$display_terms = $product_terms;
									}
									
									// Hvis vi er på bruksområde-visning, legg til bruksområdetermer
									if (strpos($field_used, 'bruksomrader') !== false && $post_terms && !is_wp_error($post_terms)) {
										$display_terms = array_merge($display_terms, $post_terms);
									} 
									
									// For kontor-visning, fjern bare Acrylicon-navnene
									elseif (strpos($field_used, 'kontor') !== false && $kontor_terms && !is_wp_error($kontor_terms)) {
										// Kontor-termer legges til bare hvis de ikke inneholder "Acrylicon" 
										foreach ($kontor_terms as $term) {
											if (strpos($term->name, 'Acrylicon') === false) {
												$display_terms[] = $term;
											}
									...
									// Legg til standard kategorier hvis ingenting annet er vist
									elseif (empty($display_terms) && $post_terms && !is_wp_error($post_terms)) {
										$display_terms = $post_terms;
									}
									
									foreach($display_terms as $term): 
										// Skip displaying tags with Acrylicon in the name
										if (strpos($term->name, 'Acrylicon') !== false) {
											continue;
										}
										?>
